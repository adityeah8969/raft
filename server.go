			...
			for i := range s.rpcClients {
				go func() {
					...
					request := &types.RequestAppendEntryRPC{
						...
						// Fill the last comitted entry here. TODO
					}
...
func (s *Server) startLeading() error {
	...
	for {
		...
		default:
			// periodic heartbeat
			// TODO: check if there is a way without initializing cancel here
	...
	for {
		...
		default:
			...
			if err != nil {
				...
				// TODO: check if we need to populate the following.
				response = &types.ResponseAppendEntryRPC{}
				responseChan <- response
				return err
			}
...
func (s *Server) Set(request *types.RequestEntry) *types.ResponseEntry {
	...
	for {
		...
		default:
			// TODO: can we consider making this a method
